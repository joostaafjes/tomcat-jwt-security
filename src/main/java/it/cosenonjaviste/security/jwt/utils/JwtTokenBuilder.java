 ...
 *
 * <tt>sub</tt> and <tt>roles</tt> values are mandatory.
 *
 * @author acomo
 *
 */
public class JwtTokenBuilder {
	...
	/**
	 * Add <tt>sub</tt> claim to JWT body
	 *
	 * @param name realm username
	 *
	 * @return {@link JwtTokenBuilder}
	 */
	public JwtTokenBuilder sub(String name) {
		return claimEntry(JwtConstants.SUB, name);
	}
	 ...
	 * @return JWT token
	 *
	 * @throws IllegalStateException if <tt>sub</tt> and <tt>roles</tt> are not provided
	 */
	public String build() {
		if (claims.containsKey(JwtConstants.SUB) && claims.containsKey(JwtConstants.ROLES)) {
			return signer.sign(claims, options);
		} else {
			throw new IllegalStateException("sub and roles claims must be added!");
		}
	}
}
